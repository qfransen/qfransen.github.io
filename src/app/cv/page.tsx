...
export default function Page() {
  return (
    <div className={"cv-container"}>
      ...
      <Section title="Education">{fullEducation()}</Section>

      <Section title="Experience">{fullExperiences()}</Section>

      <Section title="Projects">{fullProjects()}</Section>
    </div>
  );
}
...
function subsection(sectionName: string, padding_class?: string): JSX.Element {
  return (
    <>
      ...
      <h2 className={`content-wrapper border-b pb-2 ${padding_class}`}>
        {sectionName}
      </h2>
    </>
  );
    ...
    <>
      ...
      {createEducation(
        ...
        [
          ...
          "Weiler Law Group Scholarhip",
          "Midland Distinguished Scholarship",
          "Marion L. and Harriet I. Bandeen Scholarship",
          "Richard Reed Scholarship/Fellowship",
      ...
      {createEducation(
        ...
        [
          ...
          "Honors Political Science Project",
          "Co-op",
          "American Mathematical Association of Two Year Colleges (AMATYC) Competition",
        ],
        [
          "Jim and Janis Van Tiflin Mathematics Award",
...
function fullExperiences(): JSX.Element {
  return (
    <>
      ...
      {createExperience(
        "Michigan State University - Mobility Lab",
        "East Lansing, Michigan",
      ...
      {createExperience(
        "Michigan State University - HAAIL",
        "East Lansing, Michigan",
        "Research Assistant",
...
function createProject(
  ...
  return (
    <div className={"content-wrapper cv-entry"}>
      <h3>{link ? <a href={fullLink}>{title}</a> : title}</h3>
      <p>{description}</p>
    </div>
  );
}
...
function fullProjects(): JSX.Element {
  ...
  return (
    <>
      ...
      {createProject(
        "Personal Website",
        "A personal website to tell my story " +
      ...
      {createProject(
        ...
        "An online physics lab demonstration " + "on the refraction of light",
        "snell's-law",
      )}
      ...
      {createProject(
        "NFL Game Predictor",
        "Created summary statistics and model to " +
          "predict winners of NFL games in my Big Data Analysis class at MSU",
        "nfl-game-outcome-predictor",
      )}

      {createProject(
        "Backend Web and Microservices App",
        "Projects created while " +
      ...
      {createProject(
        ...
        "Autonomous lane keeping simulation " +
      ...
      {createProject(
        "Animation Editor",
        "Tool to create animations for 2D " + "characters/images",
        "animation-editor",
      )}

      {createProject(
        "Logic Game",
        "Group project in Software Design class " +
      ...
      {createProject(
        "Popping Blimps",
        "Game created during Programming II class at Delta College",
        "popping-blimps",
      )}
    </>
  );
